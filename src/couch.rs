...
fn move_players(time: Res<Time>, mut players: Query<(&PlayerInput, &mut Transform)>) {
    for (input, mut transform) in &mut players {
        ...
        let position = (transform.translation.truncate() + delta)
            .clamp(-HALF_BOUNDS, HALF_BOUNDS);
...
fn update_hud(roster: Res<PlayerRoster>, mut hud: Query<&mut Text, With<Hud>>) {
    ...
    for id in roster.players() {
        let brand = roster.brand(id);
        let status = if roster.device(id).is_some_and(|d| d.is_connected()) {
            format!("{} — bump: {}", brand.name(), brand.button_label(GamepadButton::South))
        } else {
    ...
    if roster.player_count() < roster.capacity() {
